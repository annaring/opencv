...
from test_code_ever import Pendulum
...
def control_loop():
    ...
    # limits: theta = 10 dgs / alpha = 5 dgs / theta_dot = 0.4 rads / alpha_dot = 0.3 rads
    ...
    K = np.array([-2,	13.9289162172293,	-1.46333425098854,	1.96647670223037])
    ...
    with RotaryServo() as myRot:
        ...
        while not KILL_THREAD:
            ...
            if timeStamp < 17:
                ...
                theta = myRot.motorPosition
                theta = ((1 - weight) * myRot.motorPosition) + (weight * math.radians(azi_avg))
                weight = weight + 0.0001
                weighted_azi.append(math.degrees(theta))
                if obj.lines == True:
                    past_azi.append(obj.psi_deg)
